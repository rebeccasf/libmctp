...
#ifdef pr_fmt
#undef pr_fmt
#define pr_fmt(x) "starfive test: " x
#endif
			       ...
			       void *ext)
{
	...
	mctp_binding_set_tx_enabled(ctx->starfive_pcie_binding, true);
...
static void discovery_regular_flow(struct mctp_binding_starfive_pcie *starfive_pcie,
				   struct ctx *ctx)
{
	int rc;
...
static void discovery_with_notify_flow(struct mctp_binding_starfive_pcie *starfive_pcie,
				       struct ctx *ctx)
{
	struct mctp_ctrl_req req;
	struct mctp_starfive_pcie_pkt_private pkt_prv;
	...
	discovery_regular_flow(starfive_pcie, ctx);
}
...
int main(void)
{
	...
	rc = mctp_starfive_pcie_register_default_handler(starfive_pcie);
	assert(rc == 0);

	ctx.mctp = mctp;
	ctx.starfive_pcie_binding = starfive_pcie_binding;
	ctx.discovered = false;
	ctx.bus_owner_eid = 8;
	...
	discovery_with_notify_flow(starfive_pcie, &ctx);
